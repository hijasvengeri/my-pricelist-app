...
export default function ApprovalPage() {
    ...
    //     const { id, staging_type, original_product_id, ...productData } = record;

    //     if (staging_type === 'DELETE_REQUEST') {
    //         const { error } = await supabase.from('products').delete().eq('id', original_product_id);
    //         if (error) throw error;
    //         await supabase.from('activity_logs').insert([{
    //             action_type: 'DELETED',
    //             product_name: record.items,
    ...
    //         }]);
    //     } else if (staging_type === 'NEW_PRODUCT') {
    //         const { error } = await supabase.from('products').insert([productData]);
    //         if (error) throw error;
    //         await supabase.from('activity_logs').insert([{
    //             action_type: 'APPROVED',
    //             product_name: record.items,
    //             details: 'Admin approved new product entry.',
    //             remark: remarks
    //         }]);
    //     } else if (staging_type === 'EDIT_PRODUCT') {
    //         const { error } = await supabase.from('products').update(productData).eq('id', original_product_id);
    //         if (error) throw error;
    //         await supabase.from('activity_logs').insert([{
    //             action_type: 'APPROVED',
    //             product_name: record.items,
    ...
    // const processApproval = async (record) => {
    //     // We pull out the fields that DO NOT belong in the main 'products' table.
    //     // 'productData' now only contains the valid columns like sl_no, items, prices, etc.
    //     const { 
    //         id, 
    //         staging_type, 
    //         original_product_id, 
    //         remark,       // Pulling this out fixes your 'column not found' error
    //         created_at,   // Pulling this out avoids timestamp conflicts
    //         ...productData 
    //     } = record;
    ...
    //         } 
    //         else if (staging_type === 'NEW_PRODUCT') {
    //             // Now productData does not contain 'remark'
    //             const { error } = await supabase.from('products').insert([productData]);
    //             if (error) throw error;
    ...
    //                 product_name: record.items,
    //                 details: 'Admin approved new product entry.',
    //                 remark: remarks // We use the remark from the Modal state, not the table
    //             }]);
    //         } 
    //         else if (staging_type === 'EDIT_PRODUCT') {
    //             // This handles both regular edits and 'RESUBMITTED' items
    //             const { error } = await supabase.from('products').update(productData).eq('id', original_product_id);
    //             if (error) throw error;
    ...
    //                 action_type: 'APPROVED',
    //                 product_name: record.items,
    //                 details: 'Admin approved product edits/resubmission.'
    //             }]);
    //         }

    //         // Cleanup: Delete from staging area
    //         await supabase.from('staged_products').delete().eq('id', id);
    //         message.success('Request Approved successfully');
            
    //     } catch (error) {
    //         // Rethrow so the handleConfirmAction catches it and shows the error message
    //         throw error;
    //     }
    // };
