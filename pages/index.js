...
//     setIsProcessing(true);
//     const key = 'pdf-process';
//     message.loading({ content: 'Generating PDF with clickable Index...', key });
...
//         // --- 1. GENERATE MAIN DATA TABLE ---
//         autoTable(doc, {
//             head: tableHead,
//             body: dataWithImages.map(row => {
//                 const columns = [
//                     { content: row.brand || "-", rowSpan: 1 }, 
...
//                     { content: '', data: row.base64Image } 
//                 ];
//                 if (row.rowSpan > 0) {
//                     return [{ content: row.sl_no.toString(), rowSpan: row.rowSpan }, { content: row.items || "-", rowSpan: row.rowSpan }, ...columns];
//                 }
//                 return columns;
//             }),
//             startY: 20,
...
//                     doc.setGState(new doc.GState({opacity: 0.05}));
//                     doc.addImage(logoBase64, 'PNG', pageWidth/2 - 40, pageHeight/2 , 160, 80, null, null, 45);
//                     doc.restoreGraphicsState();
//                 }
//             },
//             didDrawCell: (data) => {
//                 if (data.section === 'body' && data.column.index === 1 && data.cell.raw && data.cell.raw.content) {
//                     const slNo = data.row.cells[0].raw.content;
//                     const itemName = data.cell.raw.content;
//                     if (!itemIndexMap.find(entry => entry.item === itemName)) {
//                         itemIndexMap.push({ 
//                             slNo, 
//                             item: itemName, 
...
//                 }
//                 const raw = data.cell.raw || {};
//                 if (data.column.index === 12 && raw.data) {
//                     const imgDim = Math.min(data.cell.width - 4, data.cell.height - 4);
//                     doc.addImage(raw.data, "JPEG", data.cell.x + (data.cell.width/2) - (imgDim/2), data.cell.y + (data.cell.height/2) - (imgDim/2), imgDim, imgDim);
//                 }
//             },
//             columnStyles: { 0: { cellWidth: 12 }, 1: { cellWidth: 35 }, 12: { cellWidth: 23 } }
//         });

//         const totalDataPages = doc.internal.getNumberOfPages();
        

//         // --- 2. DRY RUN TO FIND EXACT INDEX PAGE COUNT ---
//         // This calculates exactly how many pages the index table needs
//         const tempDoc = new JsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });
//         autoTable(tempDoc, {
//             startY: 30,
//             head: [['SL No', 'Product Name', 'Page']],
//             body: itemIndexMap.map(e => [e.slNo, e.item, '0']),
//             styles: { fontSize: 11, cellPadding: 3 },
//             columnStyles: { 0: { cellWidth: 25 }, 1: { cellWidth: 200 }, 2: { cellWidth: 35 } },
//         });
//         const actualIndexPages = tempDoc.internal.getNumberOfPages();

//         // --- 3. GENERATE ACTUAL INDEX AT THE END ---
//         doc.addPage();
//         doc.setFontSize(18);
//         doc.setTextColor(0);
...
//             startY: 30,
//             head: [['SL No', 'Product Name', 'Page']],
//             // Calculate final page: original page + actualIndexPages shift
//             body: itemIndexMap.map(e => [e.slNo, e.item, `Page ${e.pageNo + actualIndexPages}`]),
//             theme: 'striped',
...
//                     const itemData = itemIndexMap[data.row.index];
//                     const targetPage = itemData.pageNo + actualIndexPages;
...
//         // --- 4. MOVE INDEX PAGES TO THE FRONT ---
//         const finalTotalPages = doc.internal.getNumberOfPages();
//         for (let i = 0; i < actualIndexPages; i++) {
//             doc.movePage(doc.internal.getNumberOfPages(), 1);
//         }

//         // --- 5. ADD PAGE FOOTERS ---
//         for (let i = 1; i <= finalTotalPages; i++) {
//             if (i <= actualIndexPages) continue; 
//             doc.setPage(i);
//             doc.setFontSize(10);
//             doc.setTextColor(150);
//             const pageWidth = doc.internal.pageSize.getWidth();
//             const pageHeight = doc.internal.pageSize.getHeight();
//             doc.text(`Page ${i} of ${finalTotalPages}`, pageWidth - 15, pageHeight - 10, { align: 'right' });
//         }

//         doc.save(`PriceList_${new Date().toLocaleDateString()}.pdf`);
//         message.success({ content: 'PDF saved with correct indexing!', key });
//     } catch (error) {
...
const handleSaveAsPdf = async () => {
    ...
    try {
        ...
        // --- CHECK PAGE COUNT FOR INDEX ---
        const totalDataPages = doc.internal.getNumberOfPages();
        let actualIndexPages = 0;

        // 🚀 ONLY ADD INDEX IF DATA IS MORE THAN 2 PAGES
        if (totalDataPages > 2) {
            message.loading({ content: 'Data is large, generating Index...', key });
            ...
            actualIndexPages = tempDoc.internal.getNumberOfPages();

            // --- 3. GENERATE ACTUAL INDEX ---
            doc.addPage();
            doc.setFontSize(18);
            doc.setTextColor(0);
            doc.text("Index / Table of Contents", 148, 20, { align: 'center' });
        ...
        const finalTotalPages = doc.internal.getNumberOfPages();
        for (let i = 1; i <= finalTotalPages; i++) {
            // Skip footer on index pages if you want
            if (actualIndexPages > 0 && i <= actualIndexPages) continue; 
