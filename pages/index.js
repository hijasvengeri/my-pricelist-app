...
for (let i = 0; i < totalIndexPages; i++) {
  ...
  // // ---------------------------------------------
  // // --- Share as Paginated Image (FIXED: Crisp Data, Faded Logo) ---   working orginal
  // // ---------------------------------------------
  // const handleShareImage = async () => {
  //   if (selectedRows.length === 0) { message.error("No items selected to share."); return; }
  //   if (!html2canvas) { message.error("Image generation library not ready."); return; }
...
//   // ---------------------------------------------
//   // --- Share as Paginated Image (UPDATED: WhatsApp Web Fallback) --- working with download and whatsapp web
//   // ---------------------------------------------
// const handleShareImage = async () => {
//     if (selectedRows.length === 0) { message.error("No items selected to share."); return; }
//     if (!html2canvas) { message.error("Image generation library not ready."); return; }
...
//     // Define element outside try so finally can access it
//     let element = null;
...
//                         background-size: 300px; opacity: 0.15; pointer-events: none; z-index: 2;
//                     "></div>
//                 ` : ''}
//                 <div style="position: relative; z-index: 2; background-color: white; padding-top: 50px;">
//                     <table class="${styles.imageTable}">
...
//             const canvas = await html2canvas(element, {
//                 scale: 3, useCORS: true, allowTaint: true, backgroundColor: '#ffffff', removeContainer: false 
//             });
...
//             imageFiles.push({
//                 file: new File([blob], `PriceList_Page_${i + 1}.jpeg`, { type: "image/jpeg" }),
//                 url: URL.createObjectURL(blob)
//             });
//         }
        
//         // 2. Direct WhatsApp Web Flow (Bypasses System Share App Prompt)
//         message.destroy(key);
//         const downloadLinks = imageFiles.map((img, index) => (
//             `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; background: #f0f2f5; padding: 8px; border-radius: 4px;">
//                 <span>Page ${index + 1}</span>
//                 <a href="${img.url}" download="${img.file.name}" style="background: #1890ff; color: white; padding: 2px 10px; border-radius: 4px; text-decoration: none; font-size: 12px;">Download</a>
//             </div>`
//         )).join('');
...
//                         Open WhatsApp Web
//                     </Button>
//                     <p style={{ fontSize: '11px', color: '#888', marginTop: '8px' }}>1. Download pages &nbsp; 2. Drag into WhatsApp</p>
//                 </div>
//             ),
//             duration: 45,
//             key: 'share-fallback'
